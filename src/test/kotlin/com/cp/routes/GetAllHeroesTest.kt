...
class GetAllHeroesTest {
    ...
    @Test
    fun `query invalid page number at getAllHeroes endpoint, assert correct response`() = testApplication {
        val pageNumber = -1
        client.get(urlString = "/boruto/heroes?page=$pageNumber").apply {
            val expectedApiResponse = ApiResponse(
                success = false,
                message = "Page number must be from 1 to 5",
                prevPage = heroRepository.getPrevPage(pageNumber),
                nextPage = heroRepository.getNextPage(pageNumber),
                heroes = getHeroesOn(pageNumber = pageNumber)
            )
    ...
    @Test
    fun `query invalid page number data type at getAllHeroes endpoint, assert correct response`() = testApplication {
        val pageNumber = "abc"
        client.get(urlString = "/boruto/heroes?page=$pageNumber").apply {
            val expectedApiResponse = ApiResponse(success = false, message = "Only Numbers Allowed")
            val actualApiResponse = Json.decodeFromString<ApiResponse>(bodyAsText())
    ...
    private fun getHeroesOn(pageNumber: Int): List<Hero> {
